...
function SignUpForm({ signUp, setSignUp }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (signUp && myRef.current && !myRef.current.contains(event.target)) {
        setSignUp(false);
      }
    };
  ...
  useEffect(
    function () {
      if (signUp) document.querySelector("body").style.overflowY = "hidden";
    ...
    try {
      ...
      if (error) {
        ...
        if (data.user.user_metadata?.email_verified) {
          dispatch({ type: "SIGN_UP_SUCCESS", payload: data.user });
          toast.success("Please check your mailbox to confirm sign up process");
          const user = await supabase.auth.getUser();
          if (user.data.user.app_metadata.pending) {
            dispatch({ type: "CHECK_CONFIRMATION", payload: false });
            toast.loading("Check your email to confirm sign up");
          } else {
            dispatch({ type: "CHECK_CONFIRMATION", payload: true });
          }
        } else {
          throw new Error("The email has signed up already");
        }
      }
    } catch (error) {
      dispatch({ type: "SIGN_UP_ERROR", payload: error });
  ...
  return (
    <div className="absolute top-0 left-0 right-0 bottom-0 animate-fade  w-full h-screen z-50">
      <div
        ref={myRef}
        className="absolute top-1/2 right-1/2 translate-x-1/2 -translate-y-1/2 bg-transparent rounded-md p-5 z-50 w-4/5 max-w-lg shadow-3xl"
      >
        ...
        >
          ...
          >
            {state.isLoading ? <PiSpinner className="m-auto" /> : "Sign Up"}
          </button>
